from binomial_tree import fit_tree
import numpy as np
...
def option_tree(S0,r,T,N,K,u,opt='european',call=True,cont_disc=False):
    ...
    #discount factor
    disc = np.exp(-self.r*T) if cont_disc==True else 1/((1+self.r)**(self.step) #to confirm the formula
    ...
    p=(1/disc-self.d)/(self.u-self.d) if cont_disc=True else (1+r-d)/(u-d)
